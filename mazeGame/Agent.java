...
public class Agent {
	...
	public Pair moveUp(Pair curLoc, boolean[][] visited) {
		...
		if(isValidLoc(x, y - 1) && !isBarrier(x, y - 1) && !visited[y - 1][x]) {
			...
			return new Pair(x, y - 1, cost, Direction.North, curLoc.getGoals());
		} else {
			return null;
		}
	}
	...
	public Pair moveDown(Pair curLoc, boolean[][] visited) {
		...
		if(isValidLoc(x, y + 1) && !isBarrier(x, y + 1) && !visited[y + 1][x]) {
			...
			return new Pair(x, y + 1, cost, Direction.South, curLoc.getGoals()); 
		} else {
			return null;
		}
	}
	...
	public Pair moveLeft(Pair curLoc, boolean[][] visited) {
		...
		if(isValidLoc(x - 1, y) && !isBarrier(x - 1, y) && !visited[y][x - 1]) {
			...
			return new Pair(x - 1, y, cost, Direction.West, curLoc.getGoals());
		} else {
			return null;
		}
	} 
	...
	public Pair moveRight(Pair curLoc, boolean[][] visited) {	
		...
		if(isValidLoc(x + 1, y) && !isBarrier(x + 1, y) && !visited[y][x + 1]) {
			...
			return new Pair(x + 1, y, cost, Direction.East, curLoc.getGoals()); 
		} else {
			return null;
		}
	}
